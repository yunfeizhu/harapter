...
アプリのテストには軽量な `harapter/testing`
を使えます。実行時の Vitest は不要で、`harapter/conformance`
の suite を使う場合だけ Vitest を追加します。Fake の決定的な結果はテスト証拠であり、実際の Provider の動作を示しません。
...
公開サブパス `harapter/testing` は `createFakeProfile`、
