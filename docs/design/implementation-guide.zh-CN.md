...
本文描述目标仓库形态和交付顺序，部分条目可能早于实现。当前行为以 Package 和 Provider
README、Export、源码、测试与已发布产物为准。本文列出的目录或 Provider 不代表已实现或已支持。
...
- 原生 Cancel 或明确不支持；
- Resume；
- Adapter 宣称的 Provider Extension；
- Adapter 宣称时的 Native Client；
- 并发限制和资源关闭。
...
- 保持 SessionRef 与原 Provider 和 Profile 绑定；
- 为新任务切换 Harness；
...
该配置只引用用户提供的 Runtime，不代表 Adapter 自动安装它们。生产实现必须从受控设置系统获取命令、端点和 Secret
Reference，且不得信任任意项目文件来启动可执行程序。
...
- SessionRef 不能跨 Provider 或 Profile 恢复；
- Optional Behavior 通过 Capability 检测；
- Native Cancel 与 Connection Abort 不混淆；
- Provider 支持时，独有功能可通过 Extension 或 Native Client 访问；
