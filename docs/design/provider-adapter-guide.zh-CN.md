...
- 仅在已观测 Capability 支持时暴露 Provider Extension、Native Client 和可选 Raw
  Event；不支持时保持空 Registry 或 `undefined`，并在文档中说明；
...
- 恢复前检查 SessionRef 的 Provider、Profile 和兼容性身份；
...
| Unexpected exit or EOF | `connection.aborted`    | 脱敏 stderr     | 中止受影响的非终态 Run     |
...
- Raw 关闭时不影响公共事件；
- 未知事件不能丢失，也不能误判成成功；
- 只有官方接口将进程退出定义为权威 Provider 失败终态时才使用
  `run.failed`；异常退出、EOF 或缺少终态权威时生成 `connection.aborted`；
...
- 不支持的 Capability；
- 无效输入；
- Session 不存在或 Provider/Profile 不匹配；
- Provider 执行失败；
- 超时；
- 连接被 Adapter 中止。
...
- Adapter 宣称的每个 Provider Extension 都具有独立类型和测试；
