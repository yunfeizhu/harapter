...
- 未知 Event 保留为 `provider` Event，并保留 `providerEventType`
  和安全、有界的摘要；可选 Raw 数据必须限制大小和结构，并进行脱敏和限流；
- 未知终态不能推断为成功；
- CLI 非零退出且缺少权威终态时映射为
  `connection.aborted`；只有官方接口将该退出定义为权威 Provider 失败时才映射为
  `run.failed`；
...
已创建 Session 仍受原 Provider、Profile 和兼容性身份约束。回滚不能让另一个 Provider 或 Profile 接管该 Session，也不能保证新 Runtime 能恢复旧 Checkpoint。
